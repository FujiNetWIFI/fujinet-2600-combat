; cbtail.asm -- the fixed half: the trampoline, the shared transport, and the
; cold stub.
;
...
CBBANK  EQU     BANKBOOT
CBHASTXT EQU    1               ; the tail carries the text primitives for now;
                                ;   if it ever runs short they move into bank 0,
                                ;   which is the only bank that draws text

; CBGOTO is not a label: cbdefs.inc gives it a fixed address and this ORG is
...
; ---------------------------------------------------------------------------
; CBGOTO -- select bank A and enter it at $1000.
;
...
; Combat wants SP = $FF at MLOOP anyway -- START does exactly this -- so here
; the reset is both free and correct.
;
...
;
; It forces a cold entry by zeroing CBENT. This console does not clear its RAM
