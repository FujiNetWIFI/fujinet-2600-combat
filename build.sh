#!/usr/bin/env bash
# build.sh -- assemble the Atari 2600 networked Combat client.
#
#   ./build.sh verify-org   the conversion gate: stock Combat, rebuilt from the
#                           DASM disassembly through tools/dasm2as.py, must be
#                           byte-identical to the cartridge dump
#
...
# 1, 3, 7 or 15 and nothing between. Combat needs three.
#
...
# colour, and several of its variations separate the pair by luminance alone --
# so on a SECAM set two players can be looking at two identical sprites. No
# amount of lockstep fixes that; it simply is not two-player Combat.
case "${TVSTD:-ntsc}" in
    ntsc) tvstd=0 ;;
    pal)  tvstd=1 ;;
    secam)
...
build.sh: refusing to build a SECAM image.
  ...
  the luminance bits. Combat tells its two tanks apart by colour alone, and
  several variations separate the pair by luminance only -- so on a SECAM set
  both players can be looking at identical sprites. That is not a sync problem
  and lockstep does not help it.
...
# ---------------- the conversion gate ----------------
#
# rom/combat.asm is the pristine 2002 DASM disassembly and is never edited.
# Every run regenerates the AS translation from it, so a converter change that
# broke a single byte could not survive here -- which is what makes this the
# anti-drift guard for everything downstream, exactly as `make verify-org` is
# in the Intellivision ports.
#
# p2bin's filler must be $FF, not 0: DASM writes zeros into the eight unused
# bytes between the last table and the reset vector, the PROM has $FF there,
# and the disassembly's own header says so.
if [ "${1:-}" = "verify-org" ]; then
    python3 tools/dasm2as.py rom/combat.asm > build/combat_org.asm
    assemble combat_org build
    "$P2BIN" build/combat_org.p build/combat_org.bin -r '$F000-$F7FF' -l 255 -q
    rm -f build/combat_org.p
    cmp build/combat_org.bin rom/combat.bin
    echo "verify-org: byte-identical ($(stat -c%s build/combat_org.bin) bytes)"
    exit 0
fi
...
# combat.inc is GENERATED from the pristine disassembly on every build, through
# the same converter verify-org proves faithful and the declared patch map in
# tools/patches.py. There is no hand-maintained copy of Combat in this tree for
# a patch to drift away from.
python3 tools/dasm2as.py rom/combat.asm > build/combat_org.asm
assemble combat_org build
python3 tools/mkbanks.py build/combat_org.lst rom/combat.asm build/combat.inc
...
python3 tools/dasm2as.py --window rom/combat.asm > build/combat_win.asm
assemble combat_win build
"$P2BIN" build/combat_win.p build/combat_win.bin -r '$1000-$17FF' -l 255 -q
rm -f build/combat_win.p
...
assemble cbtail
python3 tools/mktail.py build/cbtail.lst \
...
"$P2BIN" build/cbtail.p build/cbtail.bin -r '$1800-$1FFF' -l 255 -q
rm -f build/cbtail.p

cat "${parts[@]}" build/cbtail.bin > build/combat.bin
rm -f "${parts[@]}" build/cbtail.bin

stampclaim build/combat.bin
python3 tools/checkrom_filter.py "$VCS/tools/checkrom.py" build/combat.bin
python3 tools/check_patch.py build/combat_win.bin build/combat.bin
