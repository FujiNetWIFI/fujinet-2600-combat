/* Atari 2600 Combat netplay relay -- C implementation.
 *
 * A transliteration of ../../combat_relay_server.py, which stays the
 * canonical reference.  Function names and their order in this file match the
...
#define IR_TICK                 0
#define IR_CRC                  4
/* log_crc's guard.  Combat wants the check byte present too; Video Olympics'
...
static void try_pair(int ci)
{
     ...
     * tank -- the same convention as the Intellivision family, where the
...
int main(int argc, char **argv)
{
    ...
    const char *lobby_cliurl =
        "TNFS://apps.irata.online/Atari2600/Games/Combat.bin";
    const char *region       = "us";
    const char *game_name    = "Combat";
    const char *server_name  = "Combat Netplay";
    int    lobby_appkey    = 24;
...
#undef NEEDVAL
    ...
    if (g_variation < VARIATION_MIN || g_variation > VARIATION_MAX) {
        fprintf(stderr, "combat_relay: variation must be %d-%d\n",
                VARIATION_MIN, VARIATION_MAX);
        return 1;
    }
    if (g_port < 1 || g_port > 65535) {
        fprintf(stderr, "combat_relay: port must be 1..65535\n");
        return 2;
    }
    if (g_fixed_seed < 0 || g_fixed_seed > 0xFFFF) {
        fprintf(stderr, "combat_relay: --fixed-seed must be 1..65535\n");
        return 2;
    }
    ...
    if (inet_pton(AF_INET, g_host, &sa.sin_addr) != 1) {
        ...
        if (getaddrinfo(g_host, NULL, &hints, &res) != 0 || !res) {
            fprintf(stderr, "combat_relay: cannot resolve --host %s\n", g_host);
            return 1;
        }
