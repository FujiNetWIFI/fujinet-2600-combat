# combat-relay — the C relay

A C port of `../combat_relay_server.py`. **The Python stays canonical**: it is
...
```sh
make -C server/c            # -> server/c/combat-relay
...
```sh
server/c/combat-relay --host 127.0.0.1 --port 9600 --delay 2 --variation 0
```
...
| `--host ADDR` | `0.0.0.0` | |
| `--port N` | 9600 | |
| `--delay N` | 2 | ticks; byte [3] of START. The ROM checks it against its own `CBD` and falls back to local play on a mismatch, so 2 is the only value this ROM accepts |
| `--variation N` | 0 | 0–26 |
...
`tools/combat_client_sim.py` reads it too. The format is the Python's, exactly:
...
`src/combat-relay.c` keeps the Python's function names *and their order in the
