#!/usr/bin/env python3
"""combat_relay_server.py -- the relay for networked Atari 2600 Combat.
...
split frame in -- the 128 bytes of console RAM are Combat's, and the reply
...
Usage: combat_relay_server.py [--port 9600] [--delay 2] [--lobby-url ...]
"""
...
def main():
    ...
    ap.add_argument("--variation", type=int, default=0,
                    help="Combat game variation the match starts on, 0-26")
    ...
    ap.add_argument("--lobby-client-url",
                    default="TNFS://apps.irata.online/Atari2600/Games/Combat.bin")
    ap.add_argument("--game-name", default="Combat")
    ap.add_argument("--server-name", default="Combat Netplay")
    ap.add_argument("--region", default="us")
    args = ap.parse_args()
    if not 0 <= args.variation <= 26:
        sys.exit("combat_relay: variation must be 0-26")
    Server(args).run()
