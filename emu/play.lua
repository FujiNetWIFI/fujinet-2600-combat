...
-- HAD NEVER MOVED A JOYSTICK, so the only inputs it ever exercised were the two
...
local CBENT, CBTICK, CBERR, CBNST = 0xE6, 0xE9, 0xE8, 0xEA
local CLOCK = 0x86
...
_G._pl_drive = emu.add_machine_frame_notifier(function()
    ...
    -- Then both players fly. KEYED TO THE EMULATOR'S FRAME COUNTER, not to any
    -- tick: this is a thumb, and a thumb is asynchronous. The two consoles get
    -- different periods so the two tanks are never doing the same thing and a
    -- swap between them would show as plainly as a desync.
    if ticks >= 45 then
        local a = host and (frames // 29) % 5 or (frames // 19) % 5
        if a > 0 then want[STICK .. DIRS[a]] = true end
        local b = host and (frames // 41) % 3 or (frames // 31) % 3
        if b == 0 then want[STICK .. "Button 1"] = true end
    end
    apply(want)
end)

-- One line per tick, at the phase-0 frame -- the only instant at which two
-- consoles are comparable, and the same instant CBCRC is sampled at. The phase
-- comes from CBENT because CLOCK is a cell ClrGam rewrites.
--
...
--
-- The notifier's counter is a video-frame sample of CBTICK, and two consoles in
...
_G._pl = sp:install_write_tap(0x2C, 0x2C, "cxclr", function(off, data, mask)
    if wlo and ticks >= wlo and ticks <= whi then
        -- adv is CBADV, the gate's own verdict for this frame: $FF ran the
        ...
        -- bytes CBMIX combined for this tick -- the local ring slot the
        ...
        local t = sp:readv_u8(CBTICK)
        local ring, loc = {}, {}
        for i = 0, 7 do
            ring[#ring + 1] = string.format("%02X", sp:readv_u8(0xED + i))
        end
        for i = 0, 3 do
            loc[#loc + 1] = string.format("%02X", sp:readv_u8(0xF5 + i))
        end
        print(string.format(
            "F t%d raw%d ph%02X adv%02X err%02X gameon%02X clk%02X fwd%02X%02X turn%02X%02X rwat%d swb%02X loc%02X ring%02X | L%s R%s",
            ticks, t, sp:readv_u8(CBENT) & 0xC0,
            sp:readv_u8(0x8F), sp:readv_u8(CBERR), sp:readv_u8(0x88),
            sp:readv_u8(CLOCK), sp:readv_u8(0x8D), sp:readv_u8(0x8E),
            sp:readv_u8(0x93), sp:readv_u8(0x94), sp:readv_u8(0xEC),
            sp:readv_u8(0xD0), sp:readv_u8(0xF5 + (t & 3)),
            sp:readv_u8(0xED + (t & 7)),
            table.concat(loc, ""), table.concat(ring, "")))
    end
    if (sp:readv_u8(CBENT) & 0xC0) ~= 0x40 then return end
    nb = nb + 1
    if inject and not injected and nb >= inject then
        injected = true
        sp:write_u8(0xA4, (sp:readv_u8(0xA4) + 1) & 0xFF)   -- TankY0
        print(string.format("INJECT tick %d: TankY0 nudged by one scanline", nb))
    end
    local b = {}
    for a = 0x80, 0xB3 do b[#b + 1] = string.format("%02X", sp:readv_u8(a)) end
    b[#b + 1] = string.format("%02X", sp:readv_u8(0xDD))
    -- The ROM's own raw tick rides along so the two numbering schemes can be
    -- checked against each other rather than trusted.
    print(string.format("S %d %d %s", nb, sp:readv_u8(CBTICK),
                        table.concat(b, "")))
end)

_G._pl_stop = emu.add_machine_stop_notifier(function()
    print(string.format("PLAY tick=%d err=$%02X state=%d ent=$%02X frames=%d",
        ticks, sp:readv_u8(CBERR), sp:readv_u8(CBNST), sp:readv_u8(CBENT),
        frames))
end)
