...
--
-- probe.asm publishes a step id into CBST and the frames that step took into
-- CBFCNT. A step id is EVEN when a transaction is launched and ODD when it
...
-- Kept in step with src/cbdefs.inc and the PSTEP/PROUND equates in
-- src/probe.asm. A drifted address here reads a cell nothing writes and
-- reports a probe that never ran.
local CBST, CBERR, CBFCNT = 0xF7, 0xE8, 0xEA
local PROUND = 0xF8
...
local function report()
    ...
    print(string.format("FRAMES %d  ROUNDS %d  ERR $%02X",
                        frames, rounds, sp:readv_u8(CBERR)))
...
_G._lat_st = sp:install_write_tap(CBST, CBST, "step", function(off, data, mask)
    if data == 0xFF then
        fails = fails + 1
    ...
    -- FNRXLO/FNRXHI.
    if data % 2 == 1 and NAME[data] then note(NAME[data], sp:readv_u8(CBFCNT)) end
end)
