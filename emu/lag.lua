...
-- SWCHA/SWCHB/INPT4/INPT5 in the whole run come from CBSHIM, and the game's own
-- seven sites are gone. This answers it dynamically, which is the half a human
-- can see: a build with CBLAG=1 refreshes the shadows once every 64 frames, so
...
--
--   CBLAG=1 make combat && ./run.sh combat lag

dofile(os.getenv("A2600_EMU") .. "/det.lua")

local CLOCK, CBJOY, CBSWB = 0x86, 0xCF, 0xD0
local sp = manager.machine.devices[":maincpu"].spaces["program"]
local writes, at, frames = 0, {}, 0

_G._lg_w = sp:install_write_tap(CBJOY, CBJOY, "cbjoy", function(off, data, mask)
    writes = writes + 1
...
_G._lg_stop = emu.add_machine_stop_notifier(function()
    ...
    table.sort(phases)
    print(string.format("FRAMES %d  CBJOY writes %d (%.3f per frame)",
    ...
    if rate < 0.05 and frames > 300 then
        ...
        print("LAG FAIL: " .. string.format("%.3f writes per frame", rate)
              .. " -- a CBLAG=0 build writes every frame, so this is not the lag build")
    end
end)
