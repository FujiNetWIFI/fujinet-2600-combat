...
.PHONY: all verify-org probe echo combat frames det inputs lag sim lobby session rig rig-hold rig-play rig-repair ladder play stop clean relay-c relay-c-strict relay-c-asan server-diff
...
# The C relay. server/combat_relay_server.py stays canonical; this binary is a
