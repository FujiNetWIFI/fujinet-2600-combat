...
# relay by its script path or binary name, and fujinet-pc by the working
# directory it was started in. A bare `pkill -f fujinet` on this machine would
# take out several long-running ones that have nothing to do with this.
cd "$(dirname "$0")/.."
HERE=$(pwd)
...
pkill -f "combat_relay_server.py" 2>/dev/null || true
pkill -f "server/c/combat-relay" 2>/dev/null || true
