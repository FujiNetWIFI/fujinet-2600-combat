...
cleanup() {
    ...
    for pid in $(pgrep -x mame 2>/dev/null || true); do
        tr '\0' ' ' < "/proc/$pid/cmdline" 2>/dev/null \
            | grep -q "build/rig/${RIGDIR:-fn}combat" && kill "$pid" 2>/dev/null
    done
    for n in 1 2; do
...
for n in 1 2; do
    PLAYER="PLAYER$n" ENDPOINT="TCP://127.0.0.1:$RELAY_PORT/" \
        ./build.sh combat > "build/rig/build$n.log" 2>&1
    cp build/combat.bin "build/rig/${RIGDIR:-fn}combat$n.bin"
done
...
setsid python3 server/combat_relay_server.py --host 127.0.0.1 \
...
for n in 1 2; do
    ...
    SECS="$SECS" FUJINET_TCP="127.0.0.1:$port" \
        ./run.sh "rig/${RIGDIR:-fn}combat$n" "${RIG_LUA:-rig}" \
            < /dev/null > "build/rig/c$n.out" 2>&1 &
    sleep 1
done
wait_secs=$((SECS + 20))
for _ in $(seq "$wait_secs"); do
    pgrep -f "build/rig/${RIGDIR:-fn}combat" > /dev/null || break
    sleep 1
done
sleep 1
...
if sa and sb:
    ...
    want(ga is not None and ga.group(1) != "24",
         "SELECT changed the variation away from the default ($24)")
